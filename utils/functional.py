...
This module used to carry the pre-rendered-frame tooling (`_apply_time_filter_to_files`
and its private helpers) that `ensemble` and `vprmethods` relied on. Those wrappers now
read events through EventCV, and per-frame thinning lives in
`utils.eventcv_frames.kept_place_indices`, which derives frame times from the uniform
`dt_ms` framing instead of a sidecar timestamps file. Only the ground-truth dilation
helper remains.
"""
import numpy as np
